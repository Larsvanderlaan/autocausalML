...
outs <- lapply(c(  3, 5, 8), function(const) {
  ...
  lapply(rev(c(  250, 500 , 1000, 2500, 5000  )) ,function(n) {
    nknots = 100
    weight_screen_by_alpha <- TRUE

    out <- run_sims(const,n,5000,  formula_hal = ~ h(.) + h(.,A), num_knots = c(nknots,nknots), relaxed_fit = TRUE, screen_basis = TRUE, gen_fun = get_data_generator_univariate, lrnr_pi = Lrnr_gam$new(), lrnr_g = Lrnr_hal9001$new(formula = ~h(.)  , smoothness_orders = 1, max_degree =2, num_knots = c(nknots)), nboots=2, weight_screen_by_alpha = T)
