...
outs <- lapply(c(const), function(const) {
   lapply( (c(  n)) ,function(n) {
    ...
    out <- run_sims(const,n,nsims,  formula_hal = ~ h(.) + h(.,A) , num_knots = c(20,20), relaxed_fit = FALSE, screen_basis = TRUE, gen_fun = get_data_generator_linear, lrnr_pi = Lrnr_gam$new(), lrnr_g = Lrnr_hal9001$new(formula = ~ h(.)  , smoothness_orders = 1, max_degree =2, num_knots = c(20)), nboots=2)
