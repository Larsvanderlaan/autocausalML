...
outs <- lapply(c( 1,3,5), function(const) {
  ...
  lapply(rev(c(   250, 500, 1000, 2500  )) ,function(n) {
    ...
    out <- run_sims(const,n,250,  formula_hal = ~ h(.) + h(.,A), num_knots = c(1,1), screen_basis = TRUE, gen_fun = get_data_generator_linear_lasso, lrnr_pi = Lrnr_glmnet$new(), lrnr_g = Lrnr_hal9001$new(formula = ~h(.)  , fit_control = list(parallel = TRUE), smoothness_orders = 1, max_degree =1, num_knots = c(1)))
...
outs <- lapply(c( 3,5,8), function(const) {
  ...
  lapply(rev (c(   500, 1000,  2500 ,4000 )) ,function(n) {
    ...
    out <- run_sims(const,n,250, fit_control = fit_control, formula_hal = ~ h(.) + h(.,A), num_knots = c(nknots,nknots), screen_basis = TRUE, gen_fun = get_data_generator_nonlinear, lrnr_pi = Lrnr_gam$new(),
